    ...
    def handle_block(self, header, block_message):
        ...
        for tx in block_message.txns:
            for tx_out in tx.tx_out:
                ...
                script = Script(BytesIO(tx_out.pk_script))
...
class Script(object):
    ...
    def parse(self, script):
        """Push data and opcodes into chunks"""

        def available(stream):
            current = stream.tell()
            available = stream.seek(0, os.SEEK_END)
            stream.seek(current, os.SEEK_SET)
            return available

        script_length = available(script)

        while script.tell() < script_length:
            opcode = int.from_bytes(script.read(1), byteorder='little')

            if opcode >= 0 and opcode < OP_PUSHDATA1:
                read_length = opcode
                self.chunks.append({'type': 'data', 'value': script.read(read_length)})
